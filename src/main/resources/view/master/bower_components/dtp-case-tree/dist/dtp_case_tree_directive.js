(function () {
    ...
    module = angular.module('dtpCaseTree', []);

    module.directive('dtpCaseTree', [
        '$timeout', function ($timeout) {
            return {
                ...
                "<a ng-style=\"{'color': showColor(row.branch)}\" ng-click=\"user_clicks_branch(row.branch)\" ng-dblclick=\"update_tree_data(row.branch)\" ng-right-click=\"user_right_clicks_branch(row.branch)\" data-toggle=\"context\" data-target=\"#caseTreeMenu\" >" +
                ...
                "<span class=\"indented tree-label\" >{{ row.label }}    {{ row.allotPerson}}</span>" +
                "</a>" +
                "</li>\n</ul>",
                replace: true,
                scope: {
                ...
                link: function (scope, element, attrs) {
                    ...
                    scope.user_right_clicks_branch = function (branch) {
                        if (branch !== selected_branch) {
                            return select_branch(branch);
                        }
                    };
                    ...
                    on_treeData_change = function () {
                        ...
                        add_branch_to_list = function (level, branch, visible) {
                            ...
                            scope.tree_rows.push({
                                ...
                                branch: branch,
                                label: branch.label,
                                allotPerson:branch.allotPerson,
                                    ...
                                    return for_each_branch(function (b) {
                                        ...
                                        if (b.type == -1) {
                                            ...
                                            for (var i = 0; i <selectedBranch.length; i++) {
                                                if(selectedBranch[i].status == "whole" && selectedBranch[i].id == b.id) {
                                                    b.allotted = true;
                                                    setParentAllotted(b);
                                                }
                                            }
